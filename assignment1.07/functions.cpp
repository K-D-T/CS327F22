...
void csvParseAux(string file, string filename){
	string line;
	...
	if(infile.is_open()){
		...
		while(std::getline(infile,line)){
			if(file == "pokemon"){
				...
				i = 1;
				pokemon_moves_list.push_back(*pm1);
				delete pm1;
				pm1 = NULL;
			}
				...
				i = 1;
				pokemon_species_list.push_back(*ps1);
				delete ps1;
				ps1 = NULL;
			}
				...
				i = 1;
				pokemon_stats_list.push_back(*ps1);
				delete ps1;
				ps1 = NULL;
			}
				...
				i = 1;
				pokemon_types_list.push_back(*pt1);
				delete pt1;
				pt1 = NULL;
			}	
		}
		...
		if(pokemon_list.size() > 0)
			for(vector<pokemon>::iterator itr=pokemon_list.begin(); itr != pokemon_list.end(); ++itr)
			printPokemon(*itr);
		

		pokemon_list.clear();
		if(type_names_list.size() > 0)
			for(vector<type_names>::iterator itr=type_names_list.begin(); itr != type_names_list.end(); ++itr)
				printTypeNames(*itr);
		

		if(experience_list.size() > 0)
			for(vector<experience>::iterator itr=experience_list.begin(); itr != experience_list.end(); ++itr)
				printExperience(*itr);
		

		if(moves_list.size() > 0)
			for(vector<moves>::iterator itr=moves_list.begin(); itr != moves_list.end(); ++itr)
				printMoves(*itr);

		if(pokemon_moves_list.size() > 0){
			for(vector<pokemon_moves>::iterator itr=pokemon_moves_list.begin(); itr != pokemon_moves_list.end(); ++itr)
				printPokemonMoves(*itr);
		}

		if(pokemon_species_list.size() > 0)
			for(vector<pokemon_species>::iterator itr=pokemon_species_list.begin(); itr != pokemon_species_list.end(); ++itr)
				printPokemonSpecies(*itr);

		if(pokemon_stats_list.size() > 0)
			for(vector<pokemon_stats>::iterator itr=pokemon_stats_list.begin(); itr != pokemon_stats_list.end(); ++itr)
				printPokemonStats(*itr);

		if(stats_list.size() > 0)
			for(vector<stats>::iterator itr=stats_list.begin(); itr != stats_list.end(); ++itr)
				printStats(*itr);

		if(pokemon_types_list.size() > 0)
			for(vector<pokemon_types>::iterator itr=pokemon_types_list.begin();itr != pokemon_types_list.end(); ++itr)
