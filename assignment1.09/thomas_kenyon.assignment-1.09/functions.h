...
class pokedata{
	public:
		...
		pokemon p_list[10194];

		vector<moves> moves_list;
		moves m_list[845];

		vector<pokemon_moves> pokemon_moves_list;
		pokemon_moves pm_list[530000];

		vector<pokemon_species> pokemon_species_list;
		pokemon_species ps_list[900];

		vector<experience> experience_list;
		experience e_list[600];

		vector<type_names> type_names_list;//leave alone

		vector<pokemon_stats> pokemon_stats_list;
		pokemon_stats pst_list[6600];

		vector<stats> stats_list;// leave alon
		vector<pokemon_types> pokemon_types_list;
		pokemon_types pt_list[1700];
};
