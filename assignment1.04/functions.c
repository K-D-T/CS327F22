...
void turnControl(struct map* m){
	...
	while(1){
		...
		while(!(isEEmpty(&m->pq))){
			...
			(&m->pq,temp->t_cost);
			if(temp->c == '@'){
				m->tempPC = pcMovement(m,temp->t_x,temp->t_y,start,m->tempPC);
				print(m);
				movePC(m);
			}
			else if(temp->c == 'p'){
				...
				//print(m);
				moveTrainers(m,'p',temp->n);
			}	
			else if(temp->c == 'w'){
				trainerMovement(m,temp->t_x,temp->t_y,temp->f_x,temp->f_y,temp->n,temp->p);
				//print(m);
				moveTrainers(m,'w',temp->n);
			}
			else if(temp->c == 'e'){
				trainerMovement(m,temp->t_x,temp->t_y,temp->f_x,temp->f_y,temp->n,temp->p);
				//print(m);
				moveTrainers(m,'e',temp->n);
			}
			else if(temp->c == 'r'){
				trainerMovement(m,temp->t_x,temp->t_y,temp->f_x,temp->f_y,temp->n,temp->p);
				//print(m);
				moveTrainers(m,'r',temp->n);
			}
			else if(temp->c == 'h'){
				trainerMovement(m,temp->t_x,temp->t_y,temp->f_x,temp->f_y,temp->n,temp->p);
				//print(m);
				moveTrainers(m,'h',temp->n);
			}
			usleep(6000);
			start++;
		}
	...
	while(direction == 0){
		...
		while(valid == 0){
			...
			if(direction == 1){
				...
				if(valid == 1){
					//m->tempPC = m->field[m->pc.x-1][m->pc.y-1];
					if(start == 0){	
						if(m->pq == NULL)
						...
						//curr = pcMovement(m, m->pc.x - 1, m->pc.y - 1, start, temp);
					}
					start++;
				}
			}//up 1, same y
				...
				if(valid == 1){
					//m->tempPC = m->field[m->pc.x-1][m->pc.y];
					if(start == 0){
						if(m->pq == NULL)
						...
						//curr = pcMovement(m, m->pc.x - 1, m->pc.y, start, temp);
					}
					start++;
				}
			}//up 1, right 1
				...
				if(valid == 1){
					//m->tempPC = m->field[m->pc.x][m->pc.y+1];
					if(start == 0){
						if(m->pq == NULL)
...
;
				...
				if(valid == 1){
					...
					if(start == 0){
						...
						//curr = pcMovement(m, m->pc.x, m->pc.y - 1, start, temp);
					}//else{
						//curr = pcMovement(m, m->pc.x, m->pc.y - 1, start, curr);
...
void moveTrainers(struct map* m, char c, int n){
	...
	if(c == 'p'){
		...
		if(flag == 1){
			...
			//trainerMovement(m,ch.x-1,ch.y-1,ch.x,ch.y,n,temp);
			//m->trainerTemp[n] = temp;
			...
			//trainerMovement(m,ch.x-1,ch.y,ch.x,ch.y,n,temp);
			//m->trainerTemp[n] = temp; 
			...
			//trainerMovement(m,ch.x,ch.y+1,ch.x,ch.y,n,temp);
			//m->trainerTemp[n] = temp;
